...
class _CompoundInterestPageState extends State<CompoundInterestPage> {
  ...
  void _calculate() {
    ...
    // Add interest for end of month on the deposit? Usually deposits made end of month don't earn interest instantly, 
    // but simplified version: A = P(1+r/n)^(nt) + PMT * ...
    // Iterative approach is safer for monthly additions.
          ...
          children: [
             ...
             SizedBox(
               ...
               child: ElevatedButton(
                 ...
                 child: const Text('Hesapla', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
               ),
             ),
          ],
        ),
           ...
           Row(
             ...
             children: [
               Column(
                 ...
                 children: [
                   const Text('Yatırdığın Ana Para', style: TextStyle(color: Colors.white70, fontSize: 12)),
                   const SizedBox(height: 4),
                   Text(formatter.format(_totalInvested), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                 ],
               ),
               Column(
                 crossAxisAlignment: CrossAxisAlignment.end,
                 children: [
                   const Text('Kazanılan Faiz', style: TextStyle(color: Colors.white70, fontSize: 12)),
                   const SizedBox(height: 4),
                   Text('+${formatter.format(_totalInterest)}', style: const TextStyle(color: AppColors.success, fontWeight: FontWeight.bold)), // Keeping success color or white depending on contrast
                 ],
               ),
             ],
           ),
  ...
  Widget _buildInputFields(BuildContext context) {
    return Column(
      children: [
        _buildTextField('Başlangıç Yatırımı', _principalController, suffix: '₺'),
        const SizedBox(height: 16),
        _buildTextField('Aylık Ek Yatırım', _monthlyController, suffix: '₺'),
        const SizedBox(height: 16),
        Row(
          children: [
             Expanded(child: _buildTextField('Yıllık Faiz Oranı', _rateController, suffix: '%')),
             const SizedBox(width: 16),
             Expanded(child: _buildTextField('Süre (Yıl)', _yearsController, suffix: 'Yıl')),
          ],
        ),
      ],
    );
