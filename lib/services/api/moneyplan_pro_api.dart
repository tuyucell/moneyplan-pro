...
class MoneyPlanProApi {
  ...
  /// Dış servislerin (Binance, CoinGecko vb.) sağlık durumunu getirir
  static Future<Map<String, dynamic>> getExternalHealth() async {
    try {
      final response = await http
          .get(Uri.parse('$_baseUrl/health/external'))
          .timeout(_timeout);
      if (response.statusCode == 200) {
        return json.decode(response.body);
      }
