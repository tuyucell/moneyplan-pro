...
const DeletionRequests: React.FC = () => {
    ...
    const handleApprove = (record: DeletionRequest) => {
        confirm({
            ...
            content: `${record.users.email} kullanıcısının hesabını silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`,
            okText: 'Evet, Sil',
            okType: 'danger',
            cancelText: 'Vazgeç',
            onOk() {
    ...
    const columns: ColumnsType<DeletionRequest> = [
        {
            ...
            render: (record) => (
                <Space vertical size={0}>
                    <Text strong>{record.users?.display_name || 'İsimsiz Kullanıcı'}</Text>
                    <Text type="secondary" style={{ fontSize: '12px' }}>{record.users?.email}</Text>
                </Space>
            ),
        },
        {
